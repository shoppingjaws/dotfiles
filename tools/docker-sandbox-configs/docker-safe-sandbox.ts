...
function parseYAML(content: string): any {
	...
	for (const line of lines) {
		...
		if (nestedMatch && currentArray && nestedMatch[1] && nestedMatch[2]) {
			const key = nestedMatch[1];
			const value = nestedMatch[2];
			const lastItem = currentArray[currentArray.length - 1];
			if (lastItem) {
				lastItem[key] =
					value === "true" ? true : value === "false" ? false : value.trim();
			}
			continue;
		}
		...
		const envMatch = line.match(/^\s{2,}(\w+):\s*(.+)$/);
		if (
...
async function listAvailableRuntimes(scriptDir: string) {
	...
	try {
		...
		const yamlFiles = files
			.filter(
				(f) =>
					(f.endsWith(".yaml") || f.endsWith(".yml")) && f !== "README.yaml",
			)
			.sort();
...
async function main() {
	...
	if (separatorIndex === -1) {
		console.error(
			"Usage: docker-safe-sandbox --runtime <name> -- <command...>",
		);
		console.error("");
